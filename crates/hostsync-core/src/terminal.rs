...
pub fn launch_native_terminal(server: &Server) -> Result<(), String> {
    ...
    #[cfg(target_os = "windows")]
    {
        if let Some(pw) = password {
            let mut sshpass_args = vec!["-e".to_string(), "ssh".to_string(), "-tt".to_string(), "-o".to_string(), "PreferredAuthentications=password".to_string()];
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        let script = if let Some(pw) = password {
            ...
            format!(
                "tell application \"Terminal\" to do script \"export SSHPASS='{}'; sshpass -e ssh -tt -o PreferredAuthentications=password {}\"",
    ...
    #[cfg(target_os = "linux")]
    {
        for term in &["gnome-terminal", "konsole", "xfce4-terminal", "xterm"] {
            let result = if let Some(pw) = password {
                ...
                let mut full_args = vec!["sshpass".to_string(), "-e".to_string(), "ssh".to_string(), "-tt".to_string(), "-o".to_string(), "PreferredAuthentications=password".to_string()];
