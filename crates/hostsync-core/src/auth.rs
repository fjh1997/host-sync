...
pub async fn request_device_code() -> Result<DeviceCode, String> {
    ...
    let resp = client
        ...
        .send()
        .await
        .map_err(|e| format!("network error: {}. Check proxy settings.", e))?;
    ...
    Ok(DeviceCode {
        user_code: body["user_code"].as_str().ok_or("missing user_code")?.to_string(),
        verification_uri: body["verification_uri"].as_str().ok_or("missing verification_uri")?.to_string(),
        device_code: body["device_code"].as_str().ok_or("missing device_code")?.to_string(),
...
pub async fn poll_for_token(device_code: &DeviceCode) -> Result<String, String> {
    ...
    loop {
        ...
        if let Some(token) = body["access_token"].as_str() {
            fetch_and_save_user(&client, token).await?;
...
/// Convenience: runs both steps.
pub async fn login() -> Result<String, String> {
    let dc = request_device_code().await?;
    eprintln!("Open {} and enter code: {}", dc.verification_uri, dc.user_code);
    poll_for_token(&dc).await
}

async fn fetch_and_save_user(client: &reqwest::Client, token: &str) -> Result<(), String> {
